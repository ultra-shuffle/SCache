...
package org.scache.storage.memory
 ...
 */
private[storage] class MemoryStore(
    conf: ScacheConf,
  ...
  private val unrollMemoryThreshold: Long =
    conf.getLong("scache.storage.unrollMemoryThreshold", 1024 * 1024)
  ...
  if (maxMemory < unrollMemoryThreshold) {
    logWarning(s"Max memory ${Utils.bytesToString(maxMemory)} is less than the initial memory " +
      ...
      s"memory. Please configure Scache with more memory.")
  }
   ...
   */
  private[storage] def evictBlocksToFreeSpace(
