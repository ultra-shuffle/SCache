...
class StorageStatus(val blockManagerId: BlockManagerId, val maxMem: Long) {
  ...
  private[scache] def addBlock(blockId: BlockId, blockStatus: BlockStatus): Unit = {
  ...
  private[scache] def updateBlock(blockId: BlockId, blockStatus: BlockStatus): Unit = {
    addBlock(blockId, blockStatus)
  }

  /** Remove the given block from this storage status. */
  private[scache] def removeBlock(blockId: BlockId): Option[BlockStatus] = {
...
private[scache] object StorageUtils extends Logging {
