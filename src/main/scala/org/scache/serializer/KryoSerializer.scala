...
import org.scache.network.util.ByteUnit
import org.scache.scheduler.{CompressedMapStatus, HighlyCompressedMapStatus}
import org.scache.storage._
import org.scache.util.collection.CompactBuffer
 ...
 * Spark application.
 */
class KryoSerializer(conf: ScacheConf)
  extends org.scache.serializer.Serializer
  with Logging
  with Serializable {

  private val bufferSizeKb = conf.getSizeAsKb("scache.kryoserializer.buffer", "64k")

  if (bufferSizeKb >= ByteUnit.GiB.toKiB(2)) {
    throw new IllegalArgumentException("scache.kryoserializer.buffer must be less than " +
      s"2048 mb, got: + ${ByteUnit.KiB.toMiB(bufferSizeKb)} mb.")
  }
  private val bufferSize = ByteUnit.KiB.toBytes(bufferSizeKb).toInt

  val maxBufferSizeMb = conf.getSizeAsMb("scache.kryoserializer.buffer.max", "64m").toInt
  if (maxBufferSizeMb >= ByteUnit.GiB.toMiB(2)) {
    throw new IllegalArgumentException("scache.kryoserializer.buffer.max must be less than " +
      s"2048 mb, got: + $maxBufferSizeMb mb.")
  }
  private val maxBufferSize = ByteUnit.MiB.toBytes(maxBufferSizeMb).toInt

  private val referenceTracking = conf.getBoolean("scache.kryo.referenceTracking", true)
  private val registrationRequired = conf.getBoolean("scache.kryo.registrationRequired", false)
  private val userRegistrators = conf.getString("scache.kryo.registrator", "")
    .split(',').map(_.trim)
    .filter(!_.isEmpty)
  private val classesToRegister = conf.getString("scache.kryo.classesToRegister", "")
    .split(',').map(_.trim)
    .filter(!_.isEmpty)
  ...
  def newKryo(): Kryo = {
    ...
    try {
      ...
      // Register classes given through scache.kryo.classesToRegister.
      classesToRegister
        .foreach { className => kryo.register(Class.forName(className, true, classLoader)) }
      // Allow the user to register their own classes by setting scache.kryo.registrator.
      userRegistrators
      ...
      case e: Exception =>
        throw new Exception(s"Failed to register classes with Kryo", e)
    } finally {
  ...
  private[scache] override lazy val supportsRelocationOfSerializedObjects: Boolean = {
...
private[scache] class KryoSerializerInstance(ks: KryoSerializer) extends SerializerInstance {
    ...
    try {
      ...
      case e: KryoException if e.getMessage.startsWith("Buffer overflow") =>
        throw new Exception(s"Kryo serialization failed: ${e.getMessage}. To avoid this, " +
          "increase scache.kryoserializer.buffer.max value.")
    } finally {
      releaseKryo(kryo)
    }
 ...
 */
private[scache] class KryoInputObjectInputBridge(
 ...
 */
private[scache] class KryoOutputObjectOutputBridge(
...
private object JavaIterableWrapperSerializer extends Logging {
  ...
  val wrapperClass =
    scala.collection.Iterable(1).asJava.getClass
