...
import org.scache.util.{ByteBufferInputStream, ByteBufferOutputStream, Utils}

private[scache] class JavaSerializationStream(
...
private[scache] class JavaDeserializationStream(in: InputStream, loader: ClassLoader)
...
private[scache] class JavaSerializerInstance(
 ...
 * Spark application.
 */
class JavaSerializer(conf: ScacheConf) extends Serializer with Externalizable {
  private var counterReset = conf.getInt("scache.serializer.objectStreamReset", 100)
  private var extraDebugInfo = conf.getBoolean("scache.serializer.extraDebugInfo", true)

  protected def this() = this(new ScacheConf())  // For deserialization only
