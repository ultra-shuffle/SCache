...
kill_and_wait() {
	...
	echo "Stopping ScacheMaster (pid=$pid)"
	...
	if is_pid_running "$pid"; then
		echo "ScacheMaster did not stop in ${timeout_secs}s; sending SIGKILL" >&2
